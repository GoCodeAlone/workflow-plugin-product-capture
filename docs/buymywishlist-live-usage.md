...
Use `workflow-plugin-product-capture` `v0.1.63` for the next BuyMyWishlist
...
GHCR `product-capture-browser@sha256:<digest>` emitted by the `v0.1.63`
...
After the `v0.1.63` release publishes its browser image, dispatch
